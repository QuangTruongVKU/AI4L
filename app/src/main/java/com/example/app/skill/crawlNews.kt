...
class CrawlNews {
    ...
    fun getTopNews(): List<NewsItem> {
        val url = "https://nhachannuoi.vn/chuyen-muc/kien-thuc-chan-nuoi/chan-nuoi-gia-cam/" // URL trang web tin tức
...
fun main() = runBlocking {
    ...
    while (true) {
        ...
        if (news.isNotEmpty()) {
            ...
            val items = news.map { it.title }
            val describe = news.map { it.description }
            val image = news.map { it.image }
            val link = news.map { it.link }
            println("Title: ${items}") // Hiển thị tiêu đề bài viết
            println("Description: ${describe}") // Hiển thị mô tả bài viết
            println("Image: ${image}") // Hiển thị link ảnh minh họa
            println("Link: ${link}") // Hiển thị link bài viết
            println("-----")
        } else {
